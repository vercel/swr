...
function infiniteMiddleware<Data = any, Error = any>(useSWRNext: SWRHook) {
  return (
...
// `useSWRInfinite` is a special hook that overrides the default `key` and
// `config` arguments of `useSWR`. So here we cast its type to `Middleware`.
export default withArgs<SWRInfiniteHook>(useSWRHandler, [
