...
### Mutação Global com `useSWRInfinite` [#global-mutate-with-useswrinfinite]
...
Um exemplo mostrando como você pode implementar as seguintes funcionalidades com `useSWRInfinite` [está disponível aqui](/examples/infinite-loading):
